...
EXTERNAL_ALGO_ENTRIES, EXTERNAL_ALGO_ALIASES = _discover_external_algorithms(
    SCRIPTS_ROOT,
    set(BUILT_IN_ALGOS.keys()),
)


def list_available_algorithms() -> None:
    print('Built-in algorithms:')
    for key, desc in BUILT_IN_ALGOS.items():
        print(f'  - {key}: {desc}')
    if not EXTERNAL_ALGO_ENTRIES:
        return
    print('\nExternal scripts (use slug, dotted form, or unique short name):')
    for meta in EXTERNAL_ALGO_ENTRIES:
        rel_path = meta.path.relative_to(PROJECT_ROOT)
        alias_text = ', '.join(alias for alias in meta.aliases if alias != meta.slug)
        extra = f" (aliases: {alias_text})" if alias_text else ''
        print(f'  - {meta.slug}: {rel_path}{extra}')
...
def run_convert(args) -> None:
    ...
    is_builtin_algo = algo_key in BUILT_IN_ALGOS
    if not is_builtin_algo and external_algo is None:
        print(f"Unknown algorithm '{args.algo}'. Use '-a help' to list available options.")
        list_available_algorithms()
        raise SystemExit(2)
    algo_label_for_names = external_algo.slug if external_algo else (algo_key or args.algo.lower())
    ...
    for idx, img_path in enumerate(items, start=1):
        ...
        if args.dry_run:
            preview_note = ''
            if prev_path is not None and external_algo is None:
                preview_note = f' | preview: {prev_path}'
            algo_note = ' [external script]' if external_algo is not None else ''
            print(f"[DRY] {img_path} -> {asm_path}{preview_note}{algo_note}")
            render_progress(idx)
            continue

        if external_algo is not None:
