...
from truecolor import rgb_to_hex, generate_truecolor_assembly
...
class ARMliteStyleApp:
    ...
    def _create_controls_section(self, parent: tk.Frame):
        ...
        # Separator
        sep = tk.Label(button_frame, text="│", bg=COLORS['bg_panel'], fg=COLORS['text_dim'])
        sep.pack(side=tk.LEFT, padx=8)
        
        # Truecolor checkbox (default ON)
        self.truecolor_var = tk.BooleanVar(value=True)
        truecolor_check = tk.Checkbutton(
            button_frame,
            text="True Color",
        ...
        truecolor_check.pack(side=tk.LEFT)
    ...
    def _auto_match(self):
        ...
        self._log("Optimizing weights (coarse-to-fine search)...")
        self.root.update()
        ...
        # Search ±0.5 around coarse best with 0.25 steps
    ...
    def _generate_assembly(self, output_path: str):
        ...
        # Build color grid
        if use_truecolor:
            # True color mode: use exact RGB values
            ...
            # Use lib/truecolor.py generator
            generate_truecolor_assembly(
                color_grid,
                output_path,
                image_path=self.image_path or '',
                resolution='hi' if self.pixel_mode == 4 else 'mid',
                comment=f'Pixel mode: {self.pixel_mode} ({w}x{h})',
            )
            return
