...
def _weighted_xyz_distance(a: tuple[float, float, float], b: tuple[float, float, float], weights: tuple[float, float, float]) -> float:
    dx = (a[0] - b[0]) * weights[0]
    dy = (a[1] - b[1]) * weights[1]
    dz = (a[2] - b[2]) * weights[2]
    return math.sqrt(dx * dx + dy * dy + dz * dz)


def apply_rgb_to_xyz(img: Image.Image, weights: tuple[float, float, float] = (1.0, 1.0, 1.0)) -> list[list[str]]:
    width, height = img.size
    palette_xyz = {name: _rgb_to_xyz(rgb) for name, rgb in ARMLITE_RGB.items()}
    grid: list[list[str]] = []

    for y in range(height):
        row: list[str] = []
        for x in range(width):
            xyz_src = _rgb_to_xyz(img.getpixel((x, y)))
            best_name = 'black'
            best_distance = float('inf')
            for name, xyz in palette_xyz.items():
                dist = _weighted_xyz_distance(xyz_src, xyz, weights)
...
def process_image(image_path, output_path, weights: tuple[float, float, float]):
    ...
    grid = apply_rgb_to_xyz(img, weights=weights)
    generate_assembly(grid, output_path)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='RGB→XYZ workflow renderer for ARMLite sprites'
    )
    parser.add_argument('image', help='Path to input image')
    parser.add_argument('-o', '--output', default='converted.s', help='Output assembly file path')
    parser.add_argument('--weights', default='1,1,1', help='Comma-separated weights for X,Y,Z channels (default: 1,1,1)')
